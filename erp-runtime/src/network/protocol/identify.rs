    ...
    fn from_reader(reader: &mut BytesReader, bytes: &'a [u8]) -> quick_protobuf::Result<Self> {
        ...
        while !reader.is_eof() {
            match reader.next_tag(bytes) {
                ...
                Ok(18) => message.listen_addrs.push(reader.read_bytes(bytes)?.to_owned()),
                Ok(34) => {
...
impl MessageWrite for IdentifyMessage {
    fn get_size(&self) -> usize {
        self.protocol_version.as_ref().map_or(0, |value| 1 + sizeof_len(value.len())) +
            self.agent_version.as_ref().map_or(0, |value| 1 + sizeof_len(value.len())) +
            self.public_key.as_ref().map_or(0, |value| 1 + sizeof_len(value.len())) +
            self.listen_addrs
                .iter()
                .map(|value| 1 + sizeof_len(value.len()))
                .sum::<usize>() +
            self.observed_addr.as_ref().map_or(0, |value| 1 + sizeof_len(value.len())) +
            self.protocols
                .iter()
                .map(|value| 1 + sizeof_len(value.len()))
                .sum::<usize>() +
            self.signed_peer_record.as_ref().map_or(0, |value| 1 + sizeof_len(value.len()))
    }
