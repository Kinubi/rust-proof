...
impl NodeHelloVerifier {
    pub fn verify(
        ...
        if remote.transport_peer_id.as_slice() != authenticated_transport_peer {
            return Err(RuntimeError::config("node hello transport peer id does not match authenticated session peer"));
        }
        ...
        let verifying_key_bytes = remote
            .node_public_key
            .as_slice()
            .try_into()
            .map_err(|_| RuntimeError::config("node hello public key must be a compressed 33-byte P-256 key"))?;
        let verifying_key = verifying_key_from_bytes(&verifying_key_bytes)
