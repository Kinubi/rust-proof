...
use rp_node::{
    network::message::{ AnnounceResponse, NetworkMessage },
};
...
use crate::{
    ...
    network::{
        ...
        protocol::{
            ...
            identify::{ IdentifyInfo, IDENTIFY_PROTOCOL, decode_identify, encode_identify },
            node_hello::{
                NODE_HELLO_PROTOCOL,
                NodeHello,
                NodeHelloBuilder,
...
impl SessionWorker {
    ...
    async fn run_async(self) -> Result<(), RuntimeError> {
        ...
        let mut io = stream.into_futures_io()?;
        ...
        if let Some(expected_transport_peer) = expected_transport_peer.as_ref() {
            if expected_transport_peer.as_slice() != noise_output.remote_transport_peer_id.as_slice() {
                return Err(RuntimeError::config("bootstrap peer transport identity did not match expectation"));
            }
        }

        let mut yamux_session = match role {
            ConnectionRole::Outbound => yamux::upgrade_outbound(noise_output.stream).await?,
            ConnectionRole::Inbound => yamux::upgrade_inbound(noise_output.stream).await?,
        };
        ...
        let mut remote_node_peer_opt = Some(remote_node_peer);
        loop {
            enum LoopEvent {
            ...
            match next_event {
                ...
                LoopEvent::Command(Some(SessionCommand::Disconnect)) => break,
                ...
                LoopEvent::Inbound(Ok(None)) => break,
                LoopEvent::Inbound(Err(error)) => return Err(error),
            }
        }
    ...
    S: futures::io::AsyncRead + futures::io::AsyncWrite + Unpin,
{
    ...
    if identify.transport_peer_id.as_slice() != authenticated_transport_peer {
        return Err(RuntimeError::config("identify transport peer did not match the authenticated transport session"));
    }

    request_node_hello(muxer, node_identity, transport_identity, config, authenticated_transport_peer).await
}
    ...
    S: futures::io::AsyncRead + futures::io::AsyncWrite + Unpin,
{
    ...
    while !identify_served || verified_peer.is_none() {
        let Some(mut substream) = muxer.accept_substream().await? else {
            return Err(RuntimeError::config("remote closed the connection before session setup completed"));
        };
    ...
    S: futures::io::AsyncRead + futures::io::AsyncWrite + Unpin,
{
    ...
    if identify.transport_peer_id.as_slice() != authenticated_transport_peer {
        return Err(RuntimeError::config("identify transport peer id did not match the authenticated transport session"));
    }
    Ok(identify)
}
    ...
    S: futures::io::AsyncRead + futures::io::AsyncWrite + Unpin,
{
    ...
    let response: NodeHelloResponse = read_postcard_frame(&mut substream, config.max_frame_len).await?;
    ...
    S: futures::io::AsyncRead + futures::io::AsyncWrite + Unpin,
{
    let message = NetworkMessage::from_bytes(&frame)
        .map_err(|_| RuntimeError::config("invalid outbound network frame"))?;
    ...
    match message {
        NetworkMessage::AnnounceRequest(request) => {
            ...
            let response_frame = read_length_prefixed_frame(&mut substream, config.max_frame_len).await?;
        ...
        NetworkMessage::AnnounceResponse(_) | NetworkMessage::SyncResponse(_) =>
            Err(RuntimeError::config("outbound response routing is not implemented for session commands")),
    }
}
    ...
    match protocol.as_str() {
        ...
        NODE_HELLO_PROTOCOL => {
            ...
            if remote.node_peer_id != verified_peer.node_peer_id {
                return Err(RuntimeError::config("node hello peer changed during an existing session"));
            }
            Ok(())
        }
        ANNOUNCE_PROTOCOL => {
        ...
        SYNC_PROTOCOL => {
            let frame = read_length_prefixed_frame(&mut substream, config.max_frame_len).await?;
            let _request = decode_sync_request(&frame, config.max_frame_len)?;
            Err(RuntimeError::config("inbound sync requests are not implemented yet"))
        }
    ...
    S: futures::io::AsyncRead + Unpin,
{
    ...
    loop {
        if prefix_len >= prefix.len() {
            return Err(RuntimeError::NetworkError(Error::new(ErrorKind::InvalidData, "frame length prefix exceeds u32 varint width")));
        }

        stream
            .read_exact(&mut prefix[prefix_len..=prefix_len])
            .await
...
fn decode_length_prefixed_payload<'a>(frame: &'a [u8], max_len: u32) -> Result<&'a [u8], RuntimeError> {
    crate::network::codec::length_prefixed::decode_length_prefix(frame, max_len).map(|(_, payload)| payload)
}

fn decode_length_prefixed_payload_len(prefix: &[u8], max_len: u32) -> Result<usize, RuntimeError> {
    unsigned_varint::decode::u32(prefix)
        .map(|(len, _)| len as usize)
        .map_err(|error| RuntimeError::NetworkError(Error::new(ErrorKind::InvalidData, format!("invalid frame length prefix: {error}"))))
        .and_then(|len| {
            if len > max_len as usize {
                Err(RuntimeError::NetworkError(Error::new(ErrorKind::InvalidData, "frame length exceeds configured maximum")))
            } else {
                Ok(len)
            }
        })
...
fn send_session_event(session_event_tx: &SessionEventTx, event: SessionEvent) -> Result<(), RuntimeError> {
    session_event_tx
