    ...
    if payload.len() > (max_len as usize) {
        return Err(
            RuntimeError::NetworkError(
                Error::new(ErrorKind::InvalidData, "payload exceeds configured frame length limit")
            )
        );
    }
...
pub fn decode_length_prefix(input: &[u8], max_len: u32) -> Result<(usize, &[u8]), RuntimeError> {
    let (payload_len, remaining) = decode
        ::u32(input)
        .map_err(|error| {
            RuntimeError::NetworkError(
                Error::new(ErrorKind::InvalidData, format!("invalid length prefix: {error}"))
            )
        })?;
    ...
    if input.len() < total_len {
        return Err(
            RuntimeError::NetworkError(
                Error::new(ErrorKind::UnexpectedEof, "frame body shorter than advertised length")
            )
        );
    }
