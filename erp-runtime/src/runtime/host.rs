use edge_executor::{LocalExecutor, block_on as edge_block_on};
...
use crate::runtime::manager::{
    NetworkCommand, NodeManager, RuntimeEvent, StorageCommand, WakeCommand,
};
...
pub fn run() -> Result<(), RuntimeError> {
    ...
    runtime_handle
        .join()
        .expect("node-runtime thread panicked")?;
...
fn create_wifi(
    ...
    let configuration = Configuration::Client(ClientConfiguration {
        ssid: ssid
            .try_into()
            .map_err(|_| RuntimeError::config(WIFI_SSID_INVALID))?,
        password: password
            .try_into()
            .map_err(|_| RuntimeError::config(WIFI_PASS_INVALID))?,
...
fn run_io_runtime(
    ...
    edge_block_on(io_executor.run(async {
        ...
        let (network_res, storage_res, wake_res) =
            futures::join!(network_worker, storage_worker, wake_worker);
