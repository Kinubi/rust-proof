...
use log::{ info, warn };
use rp_core::{
    crypto::{ Signature, Signer, SigningKey, VerifyingKey, signing_key_from_bytes },
...
pub(crate) trait ValidatorSigner {
    fn verifying_key(&self) -> Result<VerifyingKey, RuntimeError>;
    fn sign_hash(&self, hash: &[u8]) -> Result<Signature, RuntimeError>;
}
...
pub(crate) fn build_signed_block(
    ...
    state_root: [u8; 32],
    signer: &impl ValidatorSigner
) -> Result<Block, RuntimeError> {
    let mut block = Block {
        height: parent.height + 1,
        slot,
        previous_hash: parent.hash(),
        validator: signer.verifying_key()?,
        transactions,
        signature: None,
        slash_proofs,
        state_root,
    ...
    let hash = block.hash();
    block.signature = Some(signer.sign_hash(&hash)?);
    Ok(block)
}

pub(crate) fn build_probe_block() -> Result<Block, RuntimeError> {
    let blockchain = Blockchain::new().expect("probe blockchain should initialize");
    let parent = blockchain.get_latest_block();
    let signer = ProbeBlockSigner::select()?;
...
enum ProbeBlockSigner {
    Efuse(EfuseBlockSigner),
    Software(SoftwareBlockSigner),
}

impl ProbeBlockSigner {
    fn select() -> Result<Self, RuntimeError> {
        ...
        match find_ecdsa_key_block() {
            Ok(key_block) => {
                if let Some((major, minor)) = unsupported_esp32p4_ecdsa_revision() {
                    warn!(
                        target: TAG,
                        "efuse ECDSA key found in block {}, but ESP32-P4 hardware ECDSA requires chip revision >= v3.0; current chip revision is v{}.{}; falling back to software block signer",
                        key_block,
                        major,
                        minor
                    );
                    Ok(Self::Software(SoftwareBlockSigner::probe()?))
                } else {
                    info!(target: TAG, "using efuse ECDSA key block {}", key_block);
                    Ok(Self::Efuse(EfuseBlockSigner { key_block }))
                }
            }
            Err("no efuse key with ECDSA_KEY purpose found") => {
                warn!(
                    target: TAG,
                    "no efuse ECDSA key provisioned; falling back to software block signer"
                );
                Ok(Self::Software(SoftwareBlockSigner::probe()?))
            }
...
impl ValidatorSigner for ProbeBlockSigner {
    fn verifying_key(&self) -> Result<VerifyingKey, RuntimeError> {
        match self {
            Self::Efuse(signer) => signer.verifying_key(),
            Self::Software(signer) => signer.verifying_key(),
        }
    }

    fn sign_hash(&self, hash: &[u8]) -> Result<Signature, RuntimeError> {
        match self {
            Self::Efuse(signer) => signer.sign_hash(hash),
            Self::Software(signer) => signer.sign_hash(hash),
        }
    }
}

struct EfuseBlockSigner {
    key_block: sys::esp_efuse_block_t,
}

impl ValidatorSigner for EfuseBlockSigner {
    fn verifying_key(&self) -> Result<VerifyingKey, RuntimeError> {
        info!(target: TAG, "exporting validator public key from efuse block {}", self.key_block);
        load_verifying_key_from_efuse(self.key_block).map_err(RuntimeError::crypto)
    }

    fn sign_hash(&self, hash: &[u8]) -> Result<Signature, RuntimeError> {
        info!(target: TAG, "signing block hash with efuse block {}", self.key_block);
        sign_hash_with_psa(hash, self.key_block).map_err(RuntimeError::crypto)
    }
}

struct SoftwareBlockSigner {
    signing_key: SigningKey,
}

impl SoftwareBlockSigner {
    fn probe() -> Result<Self, RuntimeError> {
...
impl ValidatorSigner for SoftwareBlockSigner {
    fn verifying_key(&self) -> Result<VerifyingKey, RuntimeError> {
        Ok(self.signing_key.verifying_key().clone())
    }

    fn sign_hash(&self, hash: &[u8]) -> Result<Signature, RuntimeError> {
        info!(target: TAG, "signing block hash with software fallback signer");
...
mod tests {
    ...
    fn test_signed_block_frame_is_accepted_by_node_engine() {
        ...
        let signer = SoftwareBlockSigner::probe().unwrap();
