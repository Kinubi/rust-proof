...
impl NodeManager {
    ...
    pub async fn run(&mut self) -> Result<(), RuntimeError> {
        ...
        self.storage_tx
            .send(StorageCommand::LoadLatestSnapshot)
            .await
        ...
        loop {
            ...
            let actions = match event {
                ...
                RuntimeEvent::Tick { now_ms } => self.node_engine.step(NodeInput::Tick { now_ms }),
                ...
                RuntimeEvent::FrameReceived { peer, frame } => {
                    ...
                    self.node_engine
                        .step(NodeInput::FrameReceived { peer, frame })
                }
                RuntimeEvent::StorageLoaded {
                    block_hash,
                    state_bytes,
    ...
    async fn handle_action(&mut self, action: NodeAction) -> Result<(), RuntimeError> {
        match action {
            NodeAction::BroadcastFrame { frame } => self
                .network_tx
                .send(NetworkCommand::BroadcastFrame { frame })
                .await
                .map_err(RuntimeError::network_send),
            NodeAction::DisconnectPeer { peer } => self
                .network_tx
                .send(NetworkCommand::DisconnectPeer { peer })
                .await
                .map_err(RuntimeError::network_send),
            ...
            NodeAction::LoadSnapshot { block_hash } => self
                .storage_tx
                .send(StorageCommand::LoadSnapshot { block_hash })
                .await
                .map_err(RuntimeError::storage_send),
            NodeAction::PersistBlock { block } => self
                .storage_tx
                .send(StorageCommand::PersistBlock { block })
                .await
                .map_err(RuntimeError::storage_send),
            ...
            NodeAction::RequestBlocks {
                ...
                .await
                .map_err(RuntimeError::network_send),
            NodeAction::ScheduleWake { at_ms } => self
                .wake_tx
                .send(WakeCommand::Schedule { at_ms })
                .await
                .map_err(RuntimeError::wake_send),
            NodeAction::SendFrame { peer, frame } => self
                .network_tx
                .send(NetworkCommand::SendFrame { peer, frame })
                .await
                .map_err(RuntimeError::network_send),
        }
    }
