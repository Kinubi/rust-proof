...
pub fn encode_postcard<T>(value: &T) -> io::Result<Vec<u8>> where T: Serialize + DeserializeOwned {
    postcard
        ::to_allocvec(value)
        .map_err(|_| io::Error::new(ErrorKind::InvalidData, "failed to encode postcard payload"))
}

pub fn decode_postcard<T>(bytes: &[u8]) -> io::Result<T> where T: Serialize + DeserializeOwned {
    postcard
        ::from_bytes(bytes)
...
pub async fn read_postcard_frame<S, T>(stream: &mut S, max_len: u32) -> io::Result<T>
    where S: AsyncRead + Unpin, T: Serialize + DeserializeOwned
{
